...
def best(ctx: click.Context, activity: str, max_age: int, as_json: bool) -> None:
    ...
    if not observations:
        console.print(
            f"[yellow]No recent data (max {max_age} min). Run 'coastwatch capture --once' first.[/yellow]"
        )
        raise SystemExit(0)

    # Enrich with beach names
    ...
    table = Table(title=f"Best Beaches Right Now{filter_str}")
    ...
    for i, obs in enumerate(observations, 1):
        ...
        table.add_row(str(i), name, score, crowd, waves, weather, best_for, style=style)
